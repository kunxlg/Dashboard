...
if not df.empty and 'date' in df.columns:
    ...
    # Create empty df with expected columns for display
    df = pd.DataFrame(columns=['date', 'pnl', 'month'])

# Current month filter safely
current_month = datetime.today().strftime("%Y-%m")
if not df.empty:
    month_df = df[df['date'].dt.strftime("%Y-%m") == current_month]
else:
    month_df = pd.DataFrame(columns=df.columns)
...
if not month_df.empty:
    ...
    st.write("No data for the current month yet.")
